...
- **⚡ Functions**:
  ...
  - `def handle_ingestion_success`: 无描述
  - `def render_staging_area`: 渲染暂存区状态栏 (显示文件数、打开文件夹、刷新、清空)
  - `def render_omni_ingestion_tabs`: 渲染五大源全能摄入面板 (可复用组件)
  - `def _render_advanced_options_content`: 高级选项内部内容渲染 (Fragment 隔离，防止全量 Rerun 卡顿)
...
- **⚡ Functions**:
  - `def get_kb_embedding_dim`: 检测知识库的向量维度 (Legacy: 仅返回维度)
  - `def get_kb_model_info`: 获取知识库的模型信息 (名称和维度)
...
**📝 描述**: 知识库工具函数 - 已废弃，请使用 KBNameOptimizer.smart_generate

- **⚡ Functions**:
  - `def generate_smart_kb_name`: 已废弃的函数 - 请使用 KBNameOptimizer.smart_generate
...
### 📄 src/utils/mysql_manager.py
- **🏗️ Classes**:
  - `class MySQLManager`: 无描述
...
### 📄 src/utils/web_fetcher.py
- **🏗️ Classes**:
  - `class WebFetcher`: Handles fetching content from URLs or Search Queries and saving as Markdown files.
...
- **⚡ Functions**:
  - `def extract_real_url`: [v9.5.32] 从搜索引擎跳转链接中提取真实目标 URL
  - `def fetch_url_worker`: 多进程工作函数 - 增强错误处理
...
### 📄 src/processors/database_exporter.py
- **🏗️ Classes**:
  - `class DatabaseExporter`: [v2.5.0] 数据库导出引擎
...
### 📄 src/processors/structure_parser.py
- **🏗️ Classes**:
  - `class StructureParser`: 结构解析器 - 专门识别和解析数据字典/定义文档
...
- **⚡ Functions**:
  ...
  - `def generate_smart_kb_name`: 统一的智能知识库命名函数 - 已重构为使用 KBNameOptimizer
