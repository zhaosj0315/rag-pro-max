# CPU 保护功能说明

## 🚨 问题背景

在处理大量OCR任务时，系统CPU使用率可能达到100%，导致：
- 系统过热自动关机
- 应用无响应
- 其他程序卡死

## 🛡️ 解决方案

新增CPU使用率保护机制，确保CPU使用率不超过90%，保护系统稳定运行。

## ✨ 功能特性

### 1. 实时CPU监控
- 每0.5秒检查一次CPU使用率
- 超过90%时自动启动限流保护
- 恢复正常后自动解除限流

### 2. 智能线程调度
- 根据CPU使用率动态调整工作线程数
- CPU > 85%：减少75%线程数
- CPU > 70%：减少50%线程数
- CPU < 70%：正常线程数

### 3. 安全并行执行
- 任务提交前检查CPU使用率
- 过高时延迟提交或转为串行执行
- 自动超时保护（30秒）

### 4. 紧急保护机制
- CPU > 95%：强制休眠2秒
- CPU > 98%：降低进程优先级
- 连续高负载：自动清理内存

## 🚀 使用方法

### 方法1: 安全启动（推荐）

```bash
# 使用安全启动脚本
./start_safe.sh

# 跳过测试直接启动
./start_safe.sh --skip-test
```

### 方法2: 手动启用

```python
from src.utils.cpu_throttle import start_global_cpu_protection
start_global_cpu_protection()
```

### 方法3: 紧急修复

如果系统已经在运行且CPU过高：

```bash
# 运行紧急修复脚本
python3 cpu_protection_hotfix.py
```

## ⚙️ 配置选项

编辑 `config/cpu_protection.json`：

```json
{
  "cpu_protection": {
    "enabled": true,
    "max_cpu_percent": 90.0,    // 最大CPU使用率
    "check_interval": 0.5,      // 检查间隔(秒)
    "throttle_threshold": 85.0  // 限流阈值
  }
}
```

## 📊 监控信息

### 控制台输出
```
🛡️  CPU保护已启动，最大使用率限制为90%
⚠️  CPU使用率过高 (92.3%)，启动限流保护...
✅ CPU使用率恢复正常 (78.5%)，解除限流
```

### 日志记录
- CPU使用率历史
- 限流事件记录
- 性能影响分析

## 🧪 测试验证

### 测试CPU保护功能
```bash
python3 test_cpu_throttle.py
```

### 手动压力测试
```bash
# 创建CPU密集型任务
python3 -c "
import time
from concurrent.futures import ThreadPoolExecutor

def cpu_task():
    end = time.time() + 5
    while time.time() < end:
        sum(i*i for i in range(10000))

with ThreadPoolExecutor(max_workers=12) as executor:
    futures = [executor.submit(cpu_task) for _ in range(12)]
    for f in futures:
        f.result()
"
```

## 📈 性能影响

### 正常情况
- CPU保护开销：< 1%
- 延迟增加：几乎无感知
- 内存占用：< 10MB

### 高负载情况
- 处理速度：降低10-20%（稳定优先）
- 系统稳定性：显著提升 (防止死机)
- 用户体验：流畅可控

## 🔧 故障排除

### 问题1: CPU保护不生效
**解决方案**:
```bash
# 检查是否正确启动
python3 -c "from src.utils.cpu_throttle import is_cpu_throttling; print(f'限流状态: {is_cpu_throttling()}')"

# 重新启动保护
python3 -c "from src.utils.cpu_throttle import start_global_cpu_protection; start_global_cpu_protection()"
```

### 问题2: 处理速度太慢
**解决方案**:
```bash
# 调整CPU限制阈值
# 编辑 config/cpu_protection.json
# 将 max_cpu_percent 从 90 调整为 95
```

### 问题3: 仍然出现系统关机
**解决方案**:
```bash
# 降低CPU限制阈值
# 将 max_cpu_percent 调整为 80-85
# 减少并行线程数
# 检查其他程序CPU占用
```

## 🎯 最佳实践

### 1. 启动前检查
```bash
# 检查系统负载
top -l 1 | grep "CPU usage"
# 关闭不必要的程序
```

### 2. 合理配置
- 开发环境：CPU限制 85%
- 生产环境：CPU限制 80%
- 服务器环境：CPU限制 75%

### 3. 监控建议
- 定期查看CPU使用率历史
- 关注限流事件频率
- 优化高CPU占用的操作

## 📝 更新日志

### v2.3.1 (2025-12-11)
- ✅ 新增CPU使用率保护机制
- ✅ 智能线程调度
- ✅ 紧急保护功能
- ✅ 安全启动脚本
- ✅ 配置文件支持

### 计划功能
- [ ] GPU使用率保护
- [ ] 内存使用率保护
- [ ] 自适应性能调优
- [ ] 可视化监控面板

## 🤝 反馈建议

如果遇到问题或有改进建议，请：
1. 查看日志文件 `app_logs/`
2. 运行诊断脚本 `python3 test_cpu_throttle.py`
3. 提交Issue并附上系统信息

---

**⚠️ 重要提醒**: CPU保护功能会在高负载时降低处理速度，这是为了保护系统稳定性的必要措施。如果需要最大性能，请确保系统散热良好并关闭其他程序。
