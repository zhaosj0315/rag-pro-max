...
**版本**: v6.6.0 (Analyst Intelligence Edition)  
**更新日期**: 2026-01-15
**类型**: 工程管理规范
...
- **🛠️ 工具自检 (Tooling Check)**: 检查 `scripts/` 下的检查脚本是否适配当前主版本（避免用旧版脚本测 v6 的代码）。
- **🏷️ 版本确立 (Versioning)**: 
    - 严格遵循 [SemVer 2.0](https://semver.org/)。
    - **全域扫描**: 执行 `grep -r "version" src/`，确保代码深处（如 API 定义、UI 状态栏）版本号统一为 v6.6.0。
- **📝 变更范围 (Scope)**: 明确本次迭代的核心逻辑：智能数据分析师智能体、沉浸式指挥中心 UI、Watchdog 线程安全加固。
...
-  **.gitignore**: 检查是否有新增的临时文件（如 `business_sql_memory.json` 的测试副本）需忽略。使用 `git check-ignore -v <file>` 验证关键文件是否被正确忽略。
...
-  **CHANGELOG.md**:
  ...
  - **关键**: 必须记录 v6.6.0 引入的 **UI 互斥渲染 (Exclusive Rendering)** 逻辑。

-  **README.md**:
  - 更新顶部 Badges (Version, Coverage)。
  - 更新核心功能列表：突显“智能下钻分析”与“分析记忆库”。
  - 检查 "Quick Start" 命令是否依然有效。

#### 2. 用户层 (User Layer)

-  **USER_MANUAL.md**:
  - **UI截图**: 由于 v7.9 指挥中心 UI 的重大重构（分屏设计），必须替换所有登录页截图。
  - **参数说明**: 检查数据分析模式下的新交互逻辑说明。

-  **FAQ.md**: 针对本次更新可能引发的常见疑问（如：为什么报告只显示一次？为什么 SQL 变长了？），预置 Q&A。

#### 3. 技术层 (Technical Layer)

-  **INTERFACE_SUMMARY.md**: 更新模块统计、API 端点列表。
-  **API_DOCUMENTATION.md**: 若 API 参数或返回值变更，必须同步描述。
-  **ARCHITECTURE.md**: 更新架构图，重点描述 v6.6.0 的 **分析记忆模块** 与 **上下文注入管线**。
...
- **UI 界面**: 用户看到的 Label (e.g., "智能下钻分析", "分析记忆").
- **代码变量**: 关键配置项 Key (e.g., `report_text`, `full_analysis_context`, `business_sql_memory`).
- **文档描述**: 用户手册中的用词 (e.g., "极光战略工作坊 (Strategic Workshop)").

**v6.6.0 核心术语检查清单 (当前版本重点)**:

-  智能下钻分析 / Drill-Down Analysis / `context_injection`
-  分析记忆库 / Analysis Memory / `sql_memory`
-  互斥渲染 / Exclusive Rendering / `is_data_report`
-  极光指挥中心 / Strategic Command Center / `v7.9_ui`
-  可解释性 SQL / Explainable SQL / `cte_modular_sql`

#### 2. 真实性与合规审计 (Truth & Compliance Audit)

- **仿真数据标注**: 严查数据分析报告。如果使用了仿真数据，必须包含 “⚠️ 极光战略沙盘模拟” 字样。
- **线程安全审计**: 检查 `src/apppro.py` 头部，确保 `LocalSourcesWatcher` 补丁未被意外删除。
- **水印校验**: 确保所有导出的图表均带有项目标准水印。

#### 3. 深度清理 (Standardized Cleanup)
...
- **过期测试文件**: 如 `tests/test_report_generator_fix.py`。
- **开发过程文档**: 如 `REFACTOR_PLAN.md`, `TODO_LIST.md`, `scratchpad.txt`。
- **多余日志**: `app_logs/` 下除 `.gitkeep` 外的所有运行时日志。
