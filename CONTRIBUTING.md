...
在 v9.5.1 之后，所有涉及非结构化源导入的功能**必须**遵循「暂存区 (Staging Area)」模式：
- **解耦交互**: 严禁在 UI 逻辑中直接修改 `uploaded_path`。
- **投递机制**: 必须调用 `sync_to_staging` 将材料投递至 `task_staging_dir` 物理桶。
- **数据库物料化**: 任何数据库查询结果必须通过 `DatabaseExporter` 先流式写入为 CSV 文件，再作为普通文件参与构建，禁止直接从内存构建索引。
- **来源溯源**: 每个同步动作必须携带 `source_label`，确保终端审计日志可追踪来源（上传/目录/粘贴/快照等）。
