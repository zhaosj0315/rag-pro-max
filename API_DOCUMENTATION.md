...
RAG Pro Max v6.2.2 提供完整的RESTful API接口，支持企业级集成和自动化部署。

### 💎 v6.2.2 新型特性支持
- **全维度镜像导出**: `/export` 接口现在支持导出包含对话记录镜像、多维图表图片（PNG/HTML）及真实数据库（DB）的增强版 ZIP 资产包。
- **双语洞察感应**: 查询结果元数据中新增 `bilingual_insight` 字段，直接返回中英双语业务结论。
- **模式强制隔离**: 建模 API 现在会严格验证请求中的模式标记，防止在普通 RAG 任务中意外启动结构化建模。
