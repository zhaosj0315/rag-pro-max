...
RAG Pro Max v5.6.8 提供完整的RESTful API接口，支持企业级集成和自动化部署。

### 💎 v5.6.8 新型特性支持
- **全域建模集成**: `/query` 接口现在支持对通过全域建模（PDF/MD/CSV 混合）生成的知识库进行结构化推演查询。
- **仿真数据感应**: 接口能自动感应知识库是否包含仿真数据，并在元数据中返回相应标识。
- **路径自愈支持**: API 层集成了路径自愈逻辑，当 `kb_name` 对应路径被移动或重命名（带前缀）时，能自动尝试模糊匹配加载。
