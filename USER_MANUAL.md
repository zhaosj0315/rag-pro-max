...
### 1.2 创建知识库
1.  **选择模式**: 在侧边栏导航选择“➕ 新建知识库...”。
2.  **选择数据源**: 界面顶部提供 4 种数据源模式，点击直接切换：
    - `📂 文件上传`: 拖拽或选择本地文件 (PDF, Word, TXT 等)。
    - `📝 粘贴文本`: 直接粘贴大段文本内容，自动保存为文件。
    - `🔗 网址抓取`: 输入目标 URL，配置递归深度和页数。
    - `🔍 智能搜索`: 输入行业关键词，系统自动搜索相关高质量内容。
3.  **高级选项**: 在侧边栏展开“🔧 高级选项”，可开启 OCR、自动摘要生成或强制重建索引。
4.  **执行构建**: 配置完成后，点击侧边栏底部的 **"🚀 立即创建"** 按钮启动任务。
