...
4. **角色库管理 (Prompt Library)**: 
   - 在底部可以管理多重角色提示词（如“代码专家”、“文案策划”）。支持自定义添加新角色。

### 1.5 多会话与角色切换 (v2.7.5 新增)
- **对话顶栏**: 聊天区域顶部常驻状态栏，显示当前知识库、模型及 Context 深度。
- **角色切换**: 在顶栏下拉框中选择不同的角色（如从“默认”切换到“Coder”），AI 会立即切换回答风格。
- **新建会话**: 点击顶栏右侧或侧边栏的 **“➕ 新对话”**，可开启全新的空白会话，旧会话将自动归档至侧边栏的历史列表中。

### 1.1 侧边栏导航 (v2.7.5 优化)
侧边栏现在按功能划分为 6 个核心区域：
1. **🏠 主页**: 核心问答区域，选择知识库或开启纯对话。
2. **🎭 角色**: 全局角色库管理，定义 AI 人设。
3. **⚙️ 配置**: 模型供应商连接、Context Window 设置。
4. **📊 监控**: 系统负载、检索命中率及性能指标。
5. **🔧 工具**: 知识库详细管理入口、系统监控工具。
6. **❓ 帮助**: 快速上手指南。

### 1.2 知识库选择与纯对话
- **富信息选择器**: 下拉列表会显示 `(📄文件数 | 💾大小 | 🕒创建日期)`。
- **纯对话模式 (Pure Chat)**: 
    - 如果您不需要基于特定文档回答，只需在下拉框选择 **“💬 纯对话模式 (Pure Chat)”**。
    - 系统将自动启动纯 LLM 对话引擎，跳过知识库构建过程。
