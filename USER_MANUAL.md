...
1. [快速开始](#4-快速开始)
2. [v2.4.7 Web爬取与数据处理增强版](#2-v247-web爬取与数据处理增强版)
3. [v2.4.7 交互体验深度优化版](#3-v246-交互体验深度优化版)
4. [v2.4.7 UI交互极致优化版](#4-v245-ui交互极致优化版)

---

## 2. v2.4.7 Web爬取与数据处理增强版

### 🕷️ 网页抓取 (Web Crawling)

**智能目录锁定**:
- 无论您是进行“网址抓取”还是“智能行业搜索”，系统现在会**强制锁定**本次任务生成的抓取结果目录。
- 彻底修复了之前可能出现的“抓取了新内容，却错误使用旧历史数据构建知识库”的问题。
- **所见即所得**：您抓取了 5 个页面，知识库中就一定是这 5 个最新页面。

**高级处理选项**:
- **全链路打通**: 在抓取界面勾选的“🔧 高级选项”（如：生成摘要、提取元数据、OCR 识别），现在能 100% 准确传递给后台处理引擎。
- **自动构建**: 抓取完成后，系统会自动触发知识库构建，并应用您选择的所有高级处理逻辑（如自动生成 AI 摘要）。

### 📊 数据统计修复

**存储空间**:
- 修复了知识库详情中“💾 存储占用”显示为 `0.00 B` 的问题。
- 现在您可以查看到向量数据库在磁盘上实际占用的物理空间大小（例如 `15.2 MB`）。
...
**视图融合**:
- **单页垂直流**: 彻底取消了“统计信息”与“文档列表”的分页切换。现在，您可以在一个连续的页面中同时查看知识库统计图表和文件列表，操作路径缩短 50%。
- **高级选项全选**: 在左下角构建知识库的“🔧 高级选项”面板中，新增了 **“✅ 一键全选”** 复选框。一键即可开启/关闭 OCR、元数据提取、摘要生成等所有耗时操作。

### 📄 文档列表新特性

**列表优化**:
- **行内复选框**: 每个文件名前新增了复选框，支持批量选中进行摘要生成。
- **小步幅分页**: 默认每页显示 5 条记录（可选 10/20/50），配合 **局部刷新 (Partial Rerun)** 技术，翻页时页面不会整体闪烁，体验如丝般顺滑。

### 👁️ macOS 原生预览增强

**预览功能**:
- **非阻塞体验**: 点击列表中的 `👁️` 图标唤起 Quick Look 时，不再阻塞应用主界面。您可以一边预览文档，一边继续在 RAG Pro Max 中提问。
- **自动置顶**: 预览窗口会自动强制显示在最前端，避免被浏览器遮挡。
- **智能路径**: 即使源文件位于临时目录，系统也能通过元数据自动定位，确保预览成功率。
...
**操作说明**:
- 在任何文件列表（统计信息或详细列表）中，点击操作列的 `👁️` 图标。
- 系统将立即唤起 macOS 原生的 **Quick Look (快速查看)** 窗口。
- 预览窗口会自动置于应用最前端，您可以看完后直接按空格键或点击左上角关闭。
- **支持格式**: 只要是 macOS 支持预览的格式（PDF, 图片, 文档, 视频等）均可秒开。

### 🎨 极致压缩布局

**空间优化**:
- **单行统计**: 知识库的核心统计指标（平均片段、大小等）现已由多行调整为 **1x6 单行布局**，为您腾出了更多纵向操作空间。
- **极简管理**: 移除了侧边栏原有的横向标签页，将“重建索引”简化为标题旁的小图标 `🔄`。
- **局部刷新**: 引入交互防抖技术，现在点击复选框或执行全选时，仅文件列表局部更新，页面不再整体晃动刷新。

### 🛡️ 稳固操作体验

**布局防晃**:
- 彻底禁止了页面的横向滑动。
- 禁用了 macOS 触摸板可能触发的左右滑动手势，操作更加稳固精准。
...
**三大核心服务**:
1. **FileService** - 文件处理服务
2. **KnowledgeBaseService** - 知识库管理服务  
