...
RAG Pro Max v6.9.5 重点强化了在大规模物理净化后的**逻辑回归测试**。

### 🧪 v6.9.5 核心测试领域
- **单兵调节舱验证**: 模拟不同用户的专属 TTL，验证 SessionStore 的优先级判定逻辑。
- **依赖树完整性扫描**: 物理删除 25+ 模块后，全量扫描核心链路的 `ImportError` 风险。
- **全合规引擎压力测试**: 并发扫描海量知识库目录，验证容量水位线预警的准确性。
