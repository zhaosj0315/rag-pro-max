...
RAG Pro Max v6.7.0 重点强化了对归一化解析逻辑与权限治理链路的回归测试。

### 🧪 v6.7.0 核心测试领域
- **万能附件解析**: 验证 CSV/XLSX/PPTX 等多种格式在 `File Upload Handler` 中的解析一致性。
- **资源治理矩阵**: 验证跨角色授权、物理资产清理以及“筛选全选”逻辑的原子性。
- **终端自愈状态**: 模拟端口 8899 占用，验证强制停止与重启逻辑的有效性。
