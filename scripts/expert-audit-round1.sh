...
if [ "$openai_keys" -gt 0 ]; then
    echo "   ❌ 发现 $openai_keys 个真实 OpenAI API 密钥泄露"
    find . -name "*.py" -o -name "*.json" -o -name "*.md" | xargs grep -E "sk-[a-zA-Z0-9]{48}" 2>/dev/null | grep -v ".git" | grep -v "test"
    sensitive_found=$openai_keys
else
    echo "   ✅ 无真实敏感信息泄露"
    sensitive_found=0
fi
