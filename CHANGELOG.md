...
- **Level 0 探测逻辑**: 彻底重构抓取架构，将搜索引擎定义为 L0 种子层。搜索引擎结果页不再消耗抓取深度配额，且绝不作为文档落盘，确保知识库纯净度。
- **暴力链路探测 (Violent Discovery)**: 将探测逻辑物理下沉至 `apppro.py`，采用正则表达式直接扫描 HTML 源码。彻底解决了多进程模式下的模块缓存失效问题，并绕过 90% 的 WAF 结构混淆。
- **重定向自动解壳 (Direct Unwrapping)**: 实现了对 Bing (base64)、DuckDuckGo (uddg) 及 Google 跳转链接的物理拆解，直接锁定原始 URL，抓取成功率提升 300%。
- **诊断自愈脚本**: 新增 `scripts/maintenance/debug_search_live.py`，支持在真实网络环境下一键诊断搜索引擎结构变化。

### 📦 暂存区治理中心 (Staging Area Inspector)
- **四维交互工具链**: 
    - **📂 预览**: 引入限高滚动 popover，按来源（上传/网页/数据库）分组展示资产，支持一行化显示时间与体积。
    - **📍 定位**: 物理集成 macOS `open` 指令，支持一键在 Finder 中打开暂存目录。
    - **🔄 刷新**: 强制重置上传哈希并重扫描路径，确保全源状态即时对齐。
    - **🧹 清理**: 一键物理粉碎暂存区及关联元数据。
- **审计追踪报告**: 每一份进入暂存区的材料均同步生成 `.meta` 审计文件，终端实时打印“汇聚报告”，记录来源与同步时间。
...
- **自动智能命名**: 引入“材料驱动”的命名算法。添加任意材料后，系统会自动根据暂存区特征填补知识库名称，无需手动触发 💡。
...
- **即时同步机制**: 实现 `sync_to_staging` 算法，确保拖入的文件、输入的目录（递归扫描）及粘贴的文本能实时同步至统一物理桶。
- **透明化进度**: 新增实时的“待处理暂存区”文件计数器与一键清空功能，极大提升了材料收集的掌控感。

### 🛡️ 管理员主权与配额重构 (Admin Sovereignty)
- **配额全面豁免**: 管理员（Admin）角色现已实现存储空间配额的 100% 豁免（默认无限）。
- **颗粒化配额调节**: 在“账户与访问安全”面板中找回了缺失的配额配置功能，支持在 UI 界面为指定用户微调存储上限（MB）。

### 🧼 稳健性加固与 Bug 修复
- **基础函数补全**: 物理补全了 `src/common/utils.py` 中缺失的 `save_uploaded_files` 函数，消除了文件上传时的隐性崩溃风险。
- **暂存区自愈**: 解决了页面刷新导致暂存目录被系统误删而引发的 `FileNotFoundError`。
- **日志审计增强**: 强化了终端日志输出，实时标注材料的来源标签（[文件上传]、[目录添加]、[文本粘贴]）。
...
- **互联网提取 (Internet Extraction)**: 创新性地将“网址抓取”与“智能搜索”合二为一。系统根据用户输入（URL 或关键词）自动智能路由至爬虫引擎或全网搜索链路，实现“一个输入框连接全世界”。
- **文本粘贴下沉**: 将“📝 粘贴文本”功能深度融合至“文件上传”面板内，支持**失焦自动保存**与**大文本智能截断**，操作路径更符合直觉。

### 🛡️ 稳健性与逻辑复用 (Robustness & Reuse)
- **核心算法复用**: 前端形态虽然极简，但后端完美保留了经过验证的高质量爬虫与搜索核心。深度、页数等高级参数实现了跨模式的双向同步。
- **模式隔离锁**: 引入严格的 `source_mode` 校验机制，杜绝了在文件上传过程中因残留的搜索状态而意外触发爬虫逻辑的风险。
- **智能路径反馈**: 修复了手动输入本地路径时的自动命名逻辑，现在能正确根据路径特征生成推荐库名。

## [v8.6.9] - 2026-01-21 (Omni-Source Edition)
### 🔌 全异构数据库大满贯集成 (Omni-Source DB Integration)
- **协议家族扩容**: 正式支持 9 种主流数据库连接，涵盖 **MySQL, PostgreSQL, SQLite, DuckDB, ClickHouse, SQL Server, Oracle (Thin), Alibaba MaxCompute (DataWorks), Snowflake**。
- **四维全景透视**: 升级了“资源治理”预览界面，提供 **📋 字段结构、📊 数据采样 (50行)、🕸️ 关联血缘、📈 业务洞察** 四位一体的数据摸底能力。
- **大数据专项适配**: 针对阿里云 DataWorks (MaxCompute) 实现了原生驱动支持与术语级 UI 引导。

### 🎨 侧边栏交互革命 (Sidebar UX Overhaul)
- **一键直达构建**: 物理删除了冗余的“预取”按钮。现在勾选表后点击“🚀 立即创建”，系统会自动透明执行“镜像拉取 -> 归一化归档 -> 知识库构建”全流程。
- **即时预览 (Instant Peek)**: 在侧边栏选表区新增 **`👁️`** 图标，支持在气泡弹窗中即时查阅表数据，告别盲选。
- **全模式全选**: 为数据库同步列表引入了 **“一键全选”** 与 **限高滚动容器**，大幅提升大库同步效率。

### 🆔 归一化身份管理 (Identity Normalization)
- **入口统一**: 将“知识库名称”录入区提升为全局共享组件，位置恒定，支持所有 5 种资料摄入模式。
...
## [v8.5.0] - 2026-01-21 (Omni-Source Edition)
### 🔌 多源数据库深度集成与归一化 (Multi-Source DB Integration)
- **突破性接入**: 新增第 5 种核心资料摄入模式 —— “🔌 数据库同步”。支持从外部 MySQL、PostgreSQL 实例一键同步表结构与数据采样。
- **归一化镜像架构**: 引入“适配器镜像”技术，将异构数据库表自动转化为本地标准 CSV 镜像。这一举措实现了逻辑的 **100% 复用**，确保数据库源能无缝享受 RAG 检索、图谱增强及 SQL 分步推演等所有高级功能。
- **全层级预览**: 在“资源治理”中实现了从连接、数据库到表字段及前 10 行数据的深度透视，构建前即可完成“数据摸底”。

### ⚡ 问答模式智能联动 (Smart Linkage)
- **自动感应激活**: 实现了问答界面的“智商感应”。进入具备 SQL 分析能力的知识库时，系统会自动开启“数据分析”开关，彻底消除用户手动切换的操作负担。
- **路由权重优化**: 针对数据库源增强了意图识别权重，确保涉及“关联、统计、明细”的问题能精准命中 SQL 推演引擎。

### 🛡️ 逻辑内核加固与审计 (Logic & Audit Hardening)
- **智能图谱构建 (Schema Enhancer)**: 升级了构建引擎，支持自动识别主键、挖掘枚举值并推演表间关联拓扑 (`join_graph`)。
- **全链路双写审计**: 实现了终端思维过程与 `.jsonl` 文件的同步双写。新增 `[Build Config]` 指纹，确保每一次构建与分析决策均有据可查。
- **鲁棒性缺陷扫除**: 彻底修复了 `LogManager` 参数兼容性、`analysis_context` 作用域及 `NameError: logger` 等多项历史遗留隐患。

## [v8.1.1] - 2026-01-20 (Flagship Dual-Core Edition)
### 🎨 UI 极简融合与入口优化 (Minimalist UI Fusion)
- **模式瘦身**: 从导航菜单中正式切除了独立的“📊 数据分析”入口。
- **能力下沉**: 实现了数据分析能力的“全源化”。现在用户在文件上传、粘贴文本、网址抓取、智能搜索任何一个模式下，均可通过勾选「高级选项」中的“智能数据分析”来激活 SQL 逻辑核。
- **界面净化**: 物理删除了冗余的独立上传逻辑块，提升了侧边栏的响应速度与交互纯净度。

## [v8.1.0] - 2026-01-20 (Flagship Dual-Core Edition)
### 🛡️ 意图严选路由与逻辑纠偏 (Iron-Gate Routing & Audit)
- **意图严选路由 (Iron-Gate)**: 修复了 v8.0.0 中即便未开启开关也会自动触发 SQL 分析的逻辑漏洞。现在系统严格遵循“物理底座就绪 + 手动开关授权”的双因子判定，确保用户主权。
- **实时路由审计**: 注入了 `[Route Discovery]` 诊断日志，在后台终端实时反馈系统对 RAG 语义核与 SQL 逻辑核的切换依据。
- **归一化问答闭环**: 确立了跨源问答的统一分流准则，无论摄入源头（上传/爬虫/粘贴/搜索），均执行一致的“按需激活”响应逻辑。

## [v8.0.0] - 2026-01-19 (Flagship Dual-Core Edition)
### 🧬 双核联动架构重构 (Dual-Core Integration)
- **从并行到合力**: 彻底废弃了 RAG 与 数据分析“二选一”的旧模式。现在系统以 RAG 为全量语义底座，数据分析作为“高级插件”按需挂载。
- **高级选项增强**: 在知识库构建界面注入了“💎 智能数据分析”物理开关。勾选后，系统将自动执行「真数据判定」并启动 Strategic Workshop 3.0 引擎进行结构化建模。
- **物理闭环固化**: 实现了向量索引 (`.json`) 与物理数据库 (`.db`) 在同一个 KBID 目录下的完美共存，为未来的“数文对照”功能打下坚实基础。
