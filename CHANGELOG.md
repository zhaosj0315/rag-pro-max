...
## v2.6.1 (2025-12-26) - 界面重构版

### 🎨 界面体验优化
- **4x1 扁平化布局**: 彻底重构了“新建知识库”的数据源选择界面，移除复杂的嵌套标签页，改为直观的水平 4 选 1 布局 (`📂 文件上传`, `📝 粘贴文本`, `🔗 网址抓取`, `🔍 智能搜索`)，减少用户点击次数。
- **按钮逻辑整合**: 移除了主界面冗余的“立即创建”按钮，将所有创建操作（包括网页抓取和智能搜索）统一收敛至侧边栏底部的 **"🚀 立即创建"** 按钮，符合“先配置，后执行”的操作直觉。

### ⚙️ 配置界面全面优化
- **标准化三行紧凑布局**: 统一所有供应商（Ollama, OpenAI 等）配置为清晰的三行结构：供应商选择 → 连接配置 → 模型选择，垂直空间占用减少约40%
- **增强型 OpenAI 交互体验**: 
  - 新增模型刷新功能 (🔄 按钮)，智能发现 API 下所有可用模型
  - 支持下拉选择和手动输入双模式，告别繁琐的模型 ID 输入
- **一键配置永久保存**: 新增 "⭐ 默认" 按钮，即时持久化当前配置 (供应商、地址、API Key、模型) 到本地文件
- **界面视觉降噪**: 移除占据页面的绿色状态方框，改为右下角自动消失的 Toast 气泡通知，真正做到"用完即走"
- **向量模型同步对齐**: 向量模型配置区同步执行扁平化和气泡化调整，确保整个侧边栏视觉风格统一### ⚡ 逻辑与稳定性增强
- **防止误触机制**: 修复了输入 URL 或关键词后自动触发知识库创建的问题，现在必须显式点击侧边栏按钮才会启动任务。
- **代码健壮性修复**:
    - 修复了 Python `IndentationError` (缩进错误) 和 `NameError` (变量未定义) 问题，确保代码在各种边缘情况下也能稳定运行。
    - 解决了 Streamlit 按钮 `DuplicateElementId` 冲突，为关键交互组件添加了唯一 Key。
- **冗余代码清理**: 删除了 500+ 行重复的旧版 Tab 逻辑代码，显著降低了维护成本。

---

## v2.6.0 (2025-12-25) - 智能推荐完整版

### 🚀 核心特性
- **无限追问 (Infinite Follow-up)**: 引入 LLM 动态生成机制，基于“用户问题 + AI 回答”的双重上下文生成高质量追问，彻底解决推荐枯竭问题。
- **智能去重**: 实现了后端（历史记录比对）与前端（队列实时检测）的双重过滤，杜绝重复问题出现。
- **零结果修复**: 增强了 LLM 实例检测逻辑，支持从 `Settings` 或 `ChatEngine` 多源获取模型，并增加了日期实体提取能力。

### ⚡ 稳定性增强
- **严格模式 (Strict Mode)**: 默认拒绝生成无意义的通用模板问题（如“更多信息”），确保推荐的每一个问题都能在知识库中找到答案。
- **兜底机制 (Rescue Mechanism)**: 当知识库验证过于严格导致结果为空时，自动启用高置信度的 LLM 候选问题进行补位，保证用户体验连续性。

---

## v2.5.1 (2025-12-24) - 稳定增强版
