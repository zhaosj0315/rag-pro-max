...
- **UI 布局微调**: 知识库管理操作按钮回归经典的 **2x3 网格布局**，相比单行排列更符合视觉习惯，操作区域更宽裕。
...
- **体验优化**: 文件上传成功后的提示信息现在会显示友好的智能名称（如“PDF文档库”），而不再显示令人困惑的内部临时目录名（如`batch_1765...`）。
- **搜索配置统一**: 统一了“关键词搜索”与“网址抓取”的配置参数。现在进行关键词搜索时，也可以自定义**递归深度**和**每层页数**，从而更灵活地控制搜索结果的抓取深度和广度。