...
def render_resource_governance():
    ...
    tab_dist, tab_users, tab_roles, tab_audit = st.tabs([
        "🛡️ 资源治理", "👤 用户与会话", "🎭 角色定义", "📜 行为审计"
    ])
    
    # --- Tab 1: 资源治理 (融合版) ---
    with tab_dist:
        st.caption("全域资源控制台：统一管理知识库的物理生命周期与访问权限")
        dist_mode = st.radio("操作视角", ["📂 资源矩阵", "👤 批量授权"], horizontal=True, label_visibility="collapsed")
        ...
        if dist_mode == "👤 批量授权":
            ...
            for kb in all_kbs:
                ...
                if shared_roles: status_tags.append(f"🎭 {len(shared_roles)}个角色")
                if shared_users: status_tags.append(f"👤 {len(shared_users)}人")
            ...
            with st.container(border=True):
                ...
                status_f = fc3.multiselect("按状态过滤", ["🌐 公开", "🔒 私有", "已分享"], key="res_status_f")
            ...
            for item in asset_data:
                ...
                if status_f:
                    if "🌐 公开" in status_f and "🌐 公开" not in item['状态']: match = False
                    if "🔒 私有" in status_f and "🔒 私有" not in item['状态']: match = False
                    if "已分享" in status_f and "🔒 私有" in item['状态'] and "🌐 公开" not in item['状态']: match = False # 简单近似
            ...
            if not filtered_data:
                ...
                edited_df = st.data_editor(
                    ...
                    hide_index=True, 
                    column_config={
                        "☑️ 选择": st.column_config.CheckboxColumn(required=True),
                        "占用空间": st.column_config.TextColumn(help="磁盘物理占用"),
                    },
                    key="resource_gov_editor"
                )
