...
class MultiKBQueryEngine:
    ...
    def query(self, question: str, kb_names: List[str], embed_provider: str, 
            ...
            for future in as_completed(future_to_kb):
                ...
                try:
                    ...
                    if result and result.strip():
                        logger.log("多知识库查询", "success", f"✅ [{completed_count}/{len(kb_names)}] {kb_name}: 查询成功，结果长度 {len(result)} 字符")
                        results.append({
                            'kb_name': kb_name,
                            'content': result
                        })
        ...
        logger.log("多知识库查询", "complete", f"✅ 多知识库联合查询完成，最终答案长度: {len(integrated_result)} 字符")
    ...
    def _query_single_kb(self, question: str, kb_name: str, embed_provider: str,
        ...
        try:
            ...
            # 创建查询引擎
            logger.log("单知识库查询", "info", f"⚙️ 创建查询引擎: {kb_name} (top_k=5, tree_summarize)")
            ...
            logger.log("单知识库查询", "complete", f"✅ {kb_name} 查询完成，结果长度: {len(result)} 字符")
            return result
    ...
    def _integrate_results(self, question: str, results: List[dict]) -> str:
        ...
        final_answer = "\n".join(answer_parts)
        logger.log("结果整合", "complete", f"✅ 答案整合完成，最终长度: {len(final_answer)} 字符")
        return final_answer
