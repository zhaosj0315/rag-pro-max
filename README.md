...
### ⚙️ 配置界面优化 (v2.6.1 界面重构版)
- **标准化三行紧凑布局**: 统一所有供应商配置为清晰的三行结构 (供应商选择 → 连接配置 → 模型选择)，垂直空间占用减少约40%
- **增强型 OpenAI 交互**: 新增模型刷新功能 (🔄 按钮)，智能发现 API 下所有可用模型，支持下拉选择和手动输入双模式
- **一键配置永久保存**: 新增 "⭐ 默认" 按钮，即时持久化当前配置 (供应商、地址、API Key、模型) 到本地文件
- **界面视觉降噪**: 移除占据页面的状态方框，改为右下角自动消失的 Toast 气泡通知，真正做到"用完即走"
- **向量模型同步对齐**: 向量模型配置区同步执行扁平化和气泡化调整，确保整个侧边栏视觉风格统一
