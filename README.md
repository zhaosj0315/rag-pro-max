...
### 🛡️ 资源治理矩阵 (v6.7.0)
- **全域透视**: 深度融合物理生命周期与权限分发，实时展示资产“创建时间”与“最后修改”，治理效率提升 200%。
- **确定性批量操作**: 彻底解决表格选择状态丢失问题，支持一键“全选列表”进行物理删除或授权移交。

### 📎 万能附件解析 (Universal Uploader)
- **逻辑归一化**: 聊天附件现在与知识库构建共享 100% 相同的底层解析内核，支持 20+ 种格式（CSV/Excel/Code）的即时解析注入。
- **多图 OCR 追加**: 支持连续上传图片并自动合并识别内容，完美适配多轮图片问答场景。

### 💻 终端自愈诊断
- **服务状态感知**: 引入端口级存活校验，支持 8899 端口占用的一键强制清理与服务重启。

### 🧠 数据分析师智能体 (v6.6.5)
- **智能虚拟数据生成**: 当表只有结构没有数据时，自动生成合理的虚拟数据，确保查询始终有结果。支持 LLM 驱动生成和规则引擎备用方案。
- **按需建表 (On-Demand)**: 实现先剪枝、后建表逻辑。系统仅针对相关表进行校验，分析前摇缩短 300%。
- **表名自愈 (Schema Healing)**: 引入模糊匹配机制，自动修复 `order` -> `orders` 等命名冲突，彻底杜绝幻觉表仿真。
- **跨阶段记忆 (Context Injection)**: 实现了分析链路的上下文贯通，支持 Stage N 发现驱动 Stage N+1 下钻。
- **经验沉淀库 (Analysis Memory)**: 自动记录成功 SQL 模板，越用越聪明。

### 🕷️ 饱和式抓取引擎 (v6.6.5)
- **1:1 逻辑克隆**: 彻底对齐原生高效脚本，采用**饱和式队列模式**取代 BFS 递归，确保大型文档站 100% 抓取。
- **反反爬护航**: 智能降速 (Throttling) 配合物理级随机延迟，完美规避 WAF 防火墙。

### 🛡️ 核心修复与稳定性 (v6.6.5)
- **语法错误根治**: 彻底修复了 `data_analyst.py` 在极端环境下的 SyntaxError。
- **回复重复根治**: 修复了流式输出中的叠词问题。
- **UA 统一**: 全系统爬虫 UA 统一为 Windows Chrome 标志位。

### 🖼️ 全维度镜像导出与离线报告 (v6.2.2)
- **镜像级还原**: Markdown 对话历史 1:1 还原网页端气泡、统计信息及引用溯源。
- **全视角画廊**: 每一分析阶段同步导出 AI、对比、趋势三大核心图表图片。
- **出版级 MD**: 图片直接嵌入文档（Typora 直接渲染），支持离线查看交互式 HTML。

### 🌍 全球化业务洞察 (v6.2.2)
- **中英双语报告**: 图表上方自动生成 `CN / EN` 双语业务发现，助力国际化决策。
- **智能指标卡**: 实时汇总平均值、合计值等核心 KPI，一秒抓住数据大盘。

### 🛡️ 模式隔离与数据主权 (v5.9.5)
- **逻辑强隔离**: 非“数据分析”模式下绝不误触 SQL 推演，保持纯净文本问答。
- **零数据丢失**: 保护 `raw_sources` 物理存储不被索引清理过程误删。
