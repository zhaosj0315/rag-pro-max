...
- [v3.2.2 现代化配置中心](#v322-现代化配置中心)
- [v2.9.3 联网搜索深度增强](#v293-联网搜索深度增强)
...
...
### Q: 为什么左侧的模型单选框不见了？
**A**: 这是 v3.2.2 的布局革命。为了释放横向空间，我们将服务商切换移到了顶部的 **标签页 (Tabs)**。这让中间的配置卡片更宽敞，更适合输入长 URL 或查看长模型名。

### Q: 我可以添加不属于列表的 AI 厂商吗？
**A**: 可以。点击标签页末尾的 **“➕ 新增自定义”**，填写相关信息并保存。该厂商将作为一个新的独立标签页出现在你的配置中心，并同步到对话工具栏。

### Q: 为什么输入地址后模型列表就出来了？
**A**: 我们引入了 **Zero-Click 自动感应**。系统会监听地址栏的变化，一旦发现是有效的地址（且已填 Key），就会在后台静默发起同步，无需你手动点击刷新。
