...
- **解决**: 在 v2.4.7+ 版本中，系统会自动基于元数据显示文件记录，允许您查看和管理，即使物理文件缺失。建议重新上传文件或删除该失效记录。
...
1. **Manifest 记录过旧**: 如果知识库是 v2.4.7 之前构建的，清单中可能未记录绝对路径。
...
**A**: 这是 v2.4.7 之前的一个交互 Bug。
- **解决**: 请升级到 v2.4.7。新版本引入了 `upload_hash` 校验机制，只有当文件内容真正改变时才会触发处理，彻底解决了“上传两次”的问题。
