...
RAG Pro Max v3.2.2 在原有四层架构基础上，集成了基于 DuckDuckGo 的联网搜索增强模块，并引入了智 能质量评估系统。
...
src/
├── apppro.py                    # 🚀 主应用入口 (v2.9.3 优化)
│   ├── 搜索规划器 (Search Planner): 意图拆解与多词并行检索
│   ├── 语义防火墙 (Semantic Guard): 基于意图的相关性过滤
│   ├── 智能研究 (Deep Research): 专家会审与多维合成
│   ├── 追问面板: 实时状态指示与 Fragment 局部刷新
│   └── 全局状态控制: 增强功能开关流转
```
src/utils/
