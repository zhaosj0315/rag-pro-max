# RAG Pro Max v2.7.5 系统架构文档

**版本: v2.7.5**  
**更新日期**: 2025-12-29  
**架构层级**: 4层  
**模块总数**: 195个  
**新增模块**: file_system_utils.py  
**测试覆盖率**: 93%

## 🏗️ 总体架构

RAG Pro Max v2.7.1 在原有四层架构基础上，新增了数据取证级文件系统分析能力，实现了30+项属性挖掘和macOS深度集成。

```
┌─────────────────────────────────────────┐
│           表现层 (UI Layer)              │
│  Streamlit界面 + 60/40分栏 + 健康仪表盘  │
└─────────────────┬───────────────────────┘
                  │
┌─────────────────▼───────────────────────┐
│          服务层 (Service Layer)          │
